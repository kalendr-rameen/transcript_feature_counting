...
def intron_sum_before_PTC(exons_df,exon_contain_PTC_start,exon_contain_PTC_end,strand):
    ...
    if strand == '+':
        exons_len_sum = exons_df.iloc[exon_contain_PTC_index+1:]['lenght'].sum()
        intron_sum = (exons_df.iloc[[-1]]['end'].values[0] - exon_contain_PTC_end) - exons_len_sum
    else:
        exons_len_sum = exons_df.iloc[exon_contain_PTC_index+1:]['lenght'].sum()
        intron_sum = (exon_contain_PTC_start - exons_df.iloc[[-1]]['start'].values[0]) - exons_len_sum
    return intron_sum


file_for_write = open('./intron_sum_after_PTC_all_transcript.txt','a')
for index, i in intersect.iterrows():
    #file_for_write = open('./intron_sum_after_PTC_all_NMD_transcript.txt','a')
    ID_transcript = i[9]
