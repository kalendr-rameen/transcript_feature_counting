...
ids = pd.read_csv('/Users/ramen/Desktop/Diplom/Authumn_work_2022/Poison_vs_Protein_coding/delta_PSI_ids', names=[9])
intersect = intersect.merge(ids, on=9)
...
def GC_content_before_PTC(exons_df,PTC_mid,strand,exon_contain_PTC_start,exon_contain_PTC_end,chr_num):
    string = ''
    file_for_write = open('example_2.bed', mode='w')
    exon_index_contain_PTC = exons_df[(exons_df['start'] == exon_contain_PTC_start) & (exons_df['end'] == exon_contain_PTC_end)].index.values[0]

    before_PTC_exons = exons_df.iloc[:exon_index_contain_PTC]
    for index, i in before_PTC_exons.iterrows():
    ...
    get_fasta = subprocess.check_output("bedtools getfasta -fi ~/Desktop/Diplom/Summer_work/Genome/merge_genome.fasta -bed ./example_2.bed", shell=True)
    get_fasta_split = str(get_fasta).split('\\n')
    #print(get_fasta_split,len(get_fasta_split))
    for i in range(1,len(get_fasta_split) - 1,2):
        #print(get_fasta_split[i])
...
for index, i in intersect.iterrows():
    file_for_write = open('./GC_content_before_PTC_all_nmd_transcripts_2.txt','a')
    ID_transcript = i[9]
    PTC_mid = i[20]
    strand = i[5]
    ...
    select = GC_content_before_PTC(sel_exons,PTC_mid,strand,exon_contain_PTC_start,exon_contain_PTC_end, chr_num)
    try:
