...
def exons_num_after_PTC(exons_df,start,end):
    PTC_index = exons_df[(exons_df['start'] == start) & (exons_df['end'] == end)].index[0]
    num_exons_after_PTC = (exons_df.shape[0] - 1) - PTC_index
    return num_exons_after_PTC


for index, i in intersect.iterrows():
    file_for_write = open('./exon_num_after_PTC_all_NMD_transcript.txt','a')
    ID_transcript = i[9]
    start_exon = i[1]
    end_exon = i[2]
    ...
    select = exons_num_after_PTC(sel_exons,start_exon,end_exon)
