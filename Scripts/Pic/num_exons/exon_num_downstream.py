...
exon_num_after_PTC = pd.read_csv('Scripts/covar_count/num_exons/exon_num_after_PTC_all_NMD_transcript.txt', sep='\t', header=None, names=['Transcript_ID','exon_num_after_PTC'])
...
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(exon_num_after_PTC, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(exon_num_after_PTC, on='Transcript_ID')
...
table_unite.rename({'psi_bin_col':'deltaPSI_бин', 'exon_num_after_PTC':'количество_экзонов_после_стоп_кодона', 'KD':'нокаут'}, axis=1, inplace=True)
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['количество_экзонов_после_стоп_кодона'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['количество_экзонов_после_стоп_кодона'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['количество_экзонов_после_стоп_кодона'].values
...
s = sns.catplot(
    data=table_unite, x='количество_экзонов_после_стоп_кодона', y='deltaPSI_бин',
...
s.savefig('./количество_экзонов_после_стоп_кодона.png')