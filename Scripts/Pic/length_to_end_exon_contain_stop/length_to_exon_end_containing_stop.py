...
length_to_exon_end_containing_PTC_table = pd.read_csv('Scripts/covar_count/length_to_end_exon_contain_stop/length_to_exon_end_containing_PTC_all_NMD_transcript.txt', sep='\t', header=None, names=['Transcript_ID','length_to_exon_end'])
...
pd.merge(nmdnar_upf1,table_poison, on='id').merge(length_to_exon_end_containing_PTC_table, on='Transcript_ID')
pd.merge(nmdnar_upf1,table_protein_coding, on='id').merge(length_to_exon_end_containing_PTC_table, on='Transcript_ID')
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(length_to_exon_end_containing_PTC_table, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(length_to_exon_end_containing_PTC_table, on='Transcript_ID')
...
table_unite['psi_bin_col'] = table_unite['deltaPSIc'].apply(\
    lambda x: left_int if x <= -0.1 else (middle_int if x <= 0.1 else right_int))
table_unite.sort_values(by='psi_bin_col',inplace=True)
table_unite.rename({'psi_bin_col':'deltaPSI_бин', 'length_to_exon_end':'длина_до_конца_экзона_со_стоп_кодоном', 'KD':'нокаут'}, axis=1, inplace=True)

# exon_num_after_PTC = pd.read_csv('/Users/rameeen/Desktop/Diplom/Summer_work/Covariates/Makarich/num_exons/exon_num_after_PTC_all_NMD_transcript.txt',                                                   sep='\t', header=None,                                                  names=['Transcript_ID','exon_num_after_PTC'])
# table_unite = table_unite.merge(exon_num_after_PTC, on='Transcript_ID')
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['длина_до_конца_экзона_со_стоп_кодоном'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['длина_до_конца_экзона_со_стоп_кодоном'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['длина_до_конца_экзона_со_стоп_кодоном'].values
...
s = sns.catplot(
    data=table_unite, x='длина_до_конца_экзона_со_стоп_кодоном', y='deltaPSI_бин',
...
s.savefig('длина_до_конца_экзона_со_стоп_кодоном.png')