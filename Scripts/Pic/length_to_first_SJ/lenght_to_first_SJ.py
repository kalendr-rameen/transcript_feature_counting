...
lenght_to_first_SJ_table = pd.read_csv('Scripts/covar_count/length_to_first_SJ/length_to_first_SJ_all_NMD_transcript_strand_sel.txt', sep='\t', header=None, names=['Transcript_ID','lenght_to_first_SJ'])
...
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(lenght_to_first_SJ_table, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(lenght_to_first_SJ_table, on='Transcript_ID')
...
table_unite.rename({'psi_bin_col':'deltaPSI_бин', 'lenght_to_first_SJ':'длина_до_первого_интрона', 'KD':'нокаут'}, axis=1, inplace=True)
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['длина_до_первого_интрона'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['длина_до_первого_интрона'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['длина_до_первого_интрона'].values
...
s = sns.catplot(
    data=table_unite, x='длина_до_первого_интрона', y='deltaPSI_бин',
...
s.savefig('./длина_до_первого_интрона.png')