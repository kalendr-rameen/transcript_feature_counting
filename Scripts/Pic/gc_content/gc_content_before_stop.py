...
gc_content_before_PTC_table = pd.read_csv('Scripts/covar_count/gc_content/GC_content_before_PTC_all_nmd_transcripts_2.txt', sep='\t', header=None, names=['Transcript_ID','gc_content_before_PTC'])
...
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(gc_content_before_PTC_table, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_upf1['gc_content_before_PTC'] = table_upf1['gc_content_before_PTC'].astype('float64')

table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(gc_content_before_PTC_table, on='Transcript_ID')
table_smg6['KD'] = 'SMG6/XRN1'
table_smg6['gc_content_before_PTC'] = table_smg6['gc_content_before_PTC'].astype('float64')
...
#table_unite['Transcript_ID'].drop_duplicates().reset_index(drop=True).to_csv('delta_PSI_ids', header=False,index=False)
table_unite.rename({'psi_bin_col':'deltaPSI_бин', 'gc_content_before_PTC':'GC_контент_перед_стоп_кодоном','KD':'нокаут'}, axis=1, inplace=True)
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['GC_контент_перед_стоп_кодоном'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['GC_контент_перед_стоп_кодоном'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['GC_контент_перед_стоп_кодоном'].values
...
s = sns.catplot(
    data=table_unite, x='GC_контент_перед_стоп_кодоном', y='deltaPSI_бин',
    col='нокаут', kind='box', col_wrap=2, showfliers = False, orient='h', palette=['blue','grey','yellow']
)
s.map_dataframe(annotate)
s.set_xticklabels(rotation=0)
s.savefig('./GC_контент_перед_стоп_кодоном.png')
