...
exon_lenght_after_PTC_table = pd.read_csv('Scripts/covar_count/exon_lenght_sum/exon_lenght_sum-after_stop_all_transcript.txt', sep='\t', header=None, names=['Transcript_ID','exon_lenght_after_PTC','strand'])
...
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(exon_lenght_after_PTC_table, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(exon_lenght_after_PTC_table, on='Transcript_ID')
...
table_unite['deltaPSI_бин'] = table_unite['deltaPSIc'].apply(lambda x: left_int if x <= -0.1 else (middle_int if x <= 0.1 else right_int))
table_unite.sort_values(by='deltaPSI_бин',inplace=True)
table_unite.rename({'exon_lenght_after_PTC':'длина_экзонов_после_стоп_кодона','KD':'Нокаут'},axis=1,inplace=True)
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['длина_экзонов_после_стоп_кодона'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['длина_экзонов_после_стоп_кодона'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['длина_экзонов_после_стоп_кодона'].values
    ...
    ax.text(.8,.8, f"p-value={'{0:.4g}'.format(res_1.pvalue)}", transform=ax.transAxes, fontweight='bold')
...
s = sns.catplot(
    data=table_unite, x='длина_экзонов_после_стоп_кодона', y='deltaPSI_бин',
...
s.savefig('./длина_экзонов_после_стоп_кодона.png')