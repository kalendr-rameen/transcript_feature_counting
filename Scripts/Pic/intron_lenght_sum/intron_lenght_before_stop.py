...
intron_lenght_after_PTC_table = pd.read_csv('Scripts/covar_count/intron_lenght_sum/intron_sum_before_PTC_all_transcript.txt', sep='\t', header=None, names=['Transcript_ID','intron_lenght_after_PTC','strand'])
...
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(intron_lenght_after_PTC_table, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(intron_lenght_after_PTC_table, on='Transcript_ID')
...
table_unite.rename({'psi_bin_col':'deltaPSI_бин',\
 'intron_lenght_after_PTC':'длина_интронов_перед_стоп_кодоном', 'KD':'нокаут'}, axis=1, inplace=True)
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['длина_интронов_перед_стоп_кодоном'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['длина_интронов_перед_стоп_кодоном'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['длина_интронов_перед_стоп_кодоном'].values
...
s = sns.catplot(
    data=table_unite, x='длина_интронов_перед_стоп_кодоном', y='deltaPSI_бин',
...
s.savefig('./длина_интронов_перед_стоп_кодоном.png')