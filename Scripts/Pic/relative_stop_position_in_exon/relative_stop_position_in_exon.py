...
relative_PTC_position_in_exon_table = pd.read_csv('Scripts/covar_count/relative_stop_position_in_exon/relative_PTC_position_in_exon_all_nmd_transcripts.csv', sep='\t', header=None, names=['Transcript_ID','relative_PTC_position_in_exon'])
...
table_upf1 = pd.merge(nmdnar_upf1,table, on='id').merge(relative_PTC_position_in_exon_table, on='Transcript_ID')
table_upf1['KD'] = 'UPF1/XRN1'
table_upf1['relative_PTC_position_in_exon'] = table_upf1['relative_PTC_position_in_exon'].astype('float64')
table_smg6 = pd.merge(nmdnar_smg6,table, on='id').merge(relative_PTC_position_in_exon_table, on='Transcript_ID')
table_smg6['KD'] = 'SMG6/XRN1'
table_smg6['relative_PTC_position_in_exon'] = table_smg6['relative_PTC_position_in_exon'].astype('float64')
...
table_unite.rename({'psi_bin_col':'deltaPSI_бин', 'relative_PTC_position_in_exon':'относительная_позиция_стоп_кодона_в_экзоне', 'KD':'нокаут'}, axis=1, inplace=True)
...
def annotate(data, **kws):
    ...
    v_1 = data[data['deltaPSI_бин'] == interval_1]['относительная_позиция_стоп_кодона_в_экзоне'].values
    v_2 = data[data['deltaPSI_бин'] == interval_2]['относительная_позиция_стоп_кодона_в_экзоне'].values
    v_3 = data[data['deltaPSI_бин'] == interval_3]['относительная_позиция_стоп_кодона_в_экзоне'].values
...
s = sns.catplot(
    data=table_unite, x='относительная_позиция_стоп_кодона_в_экзоне', y='deltaPSI_бин',
    col='нокаут', kind='box', col_wrap=2, showfliers = False, orient='h', palette=['blue','grey','yellow']
)
s.map_dataframe(annotate)
s.set_xticklabels(rotation=10)
s.savefig('./относительная_позиция_стоп_кодона_в_экзоне.png')